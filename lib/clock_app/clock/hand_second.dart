...
class SecondHandPainter extends CustomPainter{
  ...
  SecondHandPainter({this.second}) : secondHandPaint = new Paint(),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.rotate(2 * pi * this.second / 60);
    Path path1 = new Path();
    Path path2 = new Path();
    path1.moveTo(0.0, -radius * 0.93);
    path1.lineTo(0.0, radius * 0.10);
    path2.addOval(new Rect.fromCircle(
      radius: 5.0, center: new Offset(0, 0)
    ));
  ...
  bool shouldRepaint(SecondHandPainter oldDelegate) {
    ...
    return this.second != oldDelegate.second;
  }
